...
def train_eval_discriminator(
    ...
    # Generator produces fake images
    with torch.no_grad():
        X = input_arrays["lr_bed_elevation"].float().to(device)
        W1 = input_arrays["height_icecap"].float().to(device)
        W2 = input_arrays["velocity"].float().to(device)
        W3 = input_arrays["mass_balance"].float().to(device)
        W4 = input_arrays["hillshade"].float().to(device)
        fake_images = g_model(x=X, w1=W1, w2=W2, w3=W3,w4=W4)
        fake_labels = torch.zeros(fake_images.shape[0], 1).to(device)

    # Real groundtruth images
    real_images = input_arrays["hr_bed_elevation"].float().to(device)
...
def train_eval_generator(
    ...
    x_topo = input_arrays["lr_bed_elevation"][:, :, 2:-2, 2:-2].float().to(device)
