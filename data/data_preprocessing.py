...
class ArcticDataloader(Dataset):
    def __init__(self,
        ...
        with open(true_crops_folder, newline='') as f:
            reader = csv.reader(f)
            next(reader)
    ...
    def __getitem__(self, idx):
        ...
        bed_elevation = self.bedmachine[:, y_1:y_2, x_1:x_2]
        ...
        assert bed_elevation.shape == (1, self.crop_size, self.crop_size), f"Patch bed elevation shape mismatch: {bed_elevation.shape}"
        ...
        hr_snow_accumulation = torch.rand((1, self.crop_size, self.crop_size))
        ...
        return {
            ...
            'velocity': velocity,
            'snow_accumulation': hr_snow_accumulation,
        }
...
if __name__ == "__main__":
    ...
    #    print(f"Crop {i}: {dataset.true_crops[i]}")
    #exit()
    ...
    # Get the BedMachine data to plot as background
    bedmachine = torch.tensor(dataset.bedmachine_data['errbed'].values.astype(np.float32))
    bedmachine = (bedmachine < 11).float()

    #new_height = bedmachine.shape[0] // 5
    #new_width = bedmachine.shape[1] // 5

    #bedmachine = F.interpolate(
    #    bedmachine.unsqueeze(0).unsqueeze(0),  
    ...
    image_types = ['hr_height_icecap', 'hr_bed_elevation', 'hr_velocity', 'hr_snow_accumulation']
    titles = ["Height Icecap", "Bed Elevation", "Velocity X", "Velocity Y", "Snow Accumulation"]

    for ax, img_type, title in zip(axes, image_types, titles):
        if img_type == 'hr_velocity':
